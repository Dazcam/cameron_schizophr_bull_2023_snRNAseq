...
rule ldsr_make_annot:
    ...
    output:  "../results/04LDSR/annotation_files/snRNAseq.{CELL_TYPE}.{GENE_WINDOW}.{CHR}.annot.gz"
    conda:   "../envs/ldsr.yml"
    message: "Creating annotation files for snRNAseq: {wildcards.CELL_TYPE}, {wildcards.GENE_WINDOW}, Chr {wildcards.CHR}"
    log:     "../results/00LOG/04LDSR/ldsr_make_annot.snRNAseq.{CELL_TYPE}.{GENE_WINDOW}.Chr{CHR}.log"
    shell:
             """
...
rule ldsr_ld_scores:
    input:   annot = "../results/04LDSR/annotation_files/snRNAseq.{CELL_TYPE}.{GENE_WINDOW}.{CHR}.annot.gz",
    ...
    output:  "../results/04LDSR/annotation_files/snRNAseq.{CELL_TYPE}.{GENE_WINDOW}.{CHR}.l2.ldscore.gz"
    conda:   "../envs/ldsr.yml"
    params:  bfile = "../resources/ldsr/reference_files/1000G_EUR_Phase3_plink/1000G.EUR.QC.{CHR}",
             ldscores = "../results/04LDSR/annotation_files/snRNAseq.{CELL_TYPE}.{GENE_WINDOW}.{CHR}",
    ...
    log:     "../results/00LOG/04LDSR/snRNAseq.{CELL_TYPE}.{GENE_WINDOW}.Chr{CHR}_ldsc.log"
    shell:
...
rule ldsr_stratified_baseline_v12:
    input:   GWAS = "../results/GWAS_for_LDSR/{GWAS}_hg19_ldsc_ready.sumstats.gz",
             LDSR = expand("../results/04LDSR/annotation_files/snRNAseq.{CELL_TYPE}.{GENE_WINDOW}.{CHR}.l2.ldscore.gz", CELL_TYPE = config["CELL_TYPES"], GENE_WINDOW = config["GENE_WINDOW"], CHR = range(1,23))
    output:  "../results/04LDSR/part_herit/baseline_v1.2/snRNAseq.{CELL_TYPE}.{GENE_WINDOW}.{GWAS}_baseline.v1.2.results"
    conda:   "../envs/ldsr.yml"
    params:  weights = "../resources/ldsr/reference_files/weights_hm3_no_hla/weights.",
             ...
             LD_anns = "../results/04LDSR/annotation_files/snRNAseq.{CELL_TYPE}.{GENE_WINDOW}.",
             out_file = "../results/04LDSR/part_herit/baseline_v1.2/snRNAseq.{CELL_TYPE}.{GENE_WINDOW}.{GWAS}_baseline.v1.2"
    message: "Running Prt Hrt with {wildcards.CELL_TYPE} {wildcards.GENE_WINDOW} and {wildcards.GWAS} GWAS"
    log:     "../results/00LOG/04LDSR/snRNAseq.{CELL_TYPE}.{GENE_WINDOW}.{GWAS}.baseline.v1.2_partHerit.log"
    shell:
...
rule ldsr_stratified_summary:
    input:   expand("../results/04LDSR/part_herit/baseline_v1.2/snRNAseq.{CELL_TYPE}.{GENE_WINDOW}.{GWAS}_baseline.v1.2.results", CELL_TYPE = config["CELL_TYPES"], GENE_WINDOW = config["GENE_WINDOW"], GWAS = config["GWAS"])
    output:  "../results/04LDSR/part_herit/baseline_v1.2/snRNAseq_LDSR_{GWAS}_baseline.v1.2_summary.tsv"
    message: "Creating summary file for {wildcards.GWAS} GWAS"
    params:  dir = "../results/04LDSR/part_herit/baseline_v1.2/",
             cell_types = "../resources/sheets/GE_celltypes.tsv"
    log:     "../results/00LOG/04LDSR/snRNAseq.{GWAS}_baseline.v1.2_partHerit.summary.log"
    shell:
             """
             ...
             for Line in $Lines
             do
             grep L2_1 ../results/04LDSR/part_herit/baseline_v1.2/snRNAseq."$Line".{wildcards.GWAS}_baseline.v1.2.results | sed "s/L2_1/$Line/g" >> {output} 2> {log}
             done
