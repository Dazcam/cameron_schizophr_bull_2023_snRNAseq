...
# Final paper plots
# Figure 1 - Level 1 cluster UMAP and gene list
...
figure_1A <- DimPlot_scCustom(seurat.shi.bc, group.by = 'cluster_level_1_new',
  ...
  ggtitle("") + annotate("shadowText", label="Fun", x= labels$x, y=labels$x)

geom_shadowtext(label = cluster_level_1_new, bg.colour='white')
...
xo <- bgWidth * diff(range(figure_1A$data$UMAP_1)) / 300
yo <- bgWidth * diff(range(figure_1A$data$UMAP_2)) / 300
for (i in theta) {
  p <- figure_1A + 
    geom_text(aes(
                x = bquote(figure_1A$data$UMAP_1 + .(cos(i) * xo)),
                y = bquote(figure_1A$data$UMAP_2 + .(sin(i) * yo)),
                label = ~text
              ),
              size = labelSize,
              color = bgColor
...
# Figure 3 - Level 2 cluster UMAPs and violin plots
...
# Supplementary level 2 figures
plot_grid(cluster_LGE_umap, cluster_LGE_vln, labels = 'AUTO', label_size = 20)
plot_grid(cluster_MGE_umap, cluster_MGE_vln, labels = 'AUTO', label_size = 20)
plot_grid(cluster_CGE_umap, cluster_CGE_vln, labels = 'AUTO', label_size = 20)
plot_grid(cluster_Progenitor_umap, cluster_Progenitor_vln, labels = 'AUTO', label_size = 20)
plot_grid(cluster_Early_InN_umap, cluster_Early_InN_vln, labels = 'AUTO', label_size = 20)

# plot_grid(cluster_Progenitor_umap, cluster_LGE_umap, cluster_MGE_umap,
#           cluster_CGE_umap, labels = 'AUTO', label_size = 20)


# For testing
# VlnPlot(seurat.shi.bc_MGE, c('SST', 'VIP', 'PVALB', 'NPY', 'NOS', 'NPY', 'CHAT', 'ACHE',
#                              'TAC1', 'ACLY', 'BRCA1', 'SLC5A7', 'SLC18A3'), stack = TRUE, flip = TRUE,
#         cols = DiscretePalette_scCustomize(num_colors = 26, palette = "ditto_seq"),
#         same.y.lims = TRUE, fill.by = 'ident', group.by = 'cluster_level_2_new') +
#   theme(axis.title.x=element_blank(), legend.position = "none")
